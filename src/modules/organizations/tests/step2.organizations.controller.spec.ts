...
describe('Organizations Controller (e2e)', () => {
  ...
  it('POST /organizations without JWT -> 401', async () => {
    const res = await request(app.getHttpServer()).post('/organizations').send({
      name: 'Test',
  ...
  it('DELETE /organizations/:id returns 404 for non-existing organization', async () => {
    ...
    const res = await request(app.getHttpServer())
      .delete('/organizations/non-existing')
      .set('Authorization', `Bearer ${token}`);
  ...
  it('GET /organizations/:id returns 404 for non-existing organization', async () => {
    ...
    const res = await request(app.getHttpServer())
      .get('/organizations/non-existing')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(404);
  });
  ...
  it('PUT /organizations/:id returns 404 for non-existing organization', async () => {
    ...
    const res = await request(app.getHttpServer())
      .put('/organizations/non-existing')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Updated' });
    expect(res.status).toBe(404);
  });
