...
describe('OrganizationRepository - Step 9: Mandatory registered_name and unified organization naming', () => {
  ...
  describe('INDIVIDUAL category organization creation', () => {
    it('should create organization with name from registered_name', async () => {
      const data = {
        ...
        middle_name: 'Michael',
        last_name: 'Doe',
        registered_name: 'John Doe Enterprises', // Mandatory for all categories
        trade_name: null,
      ...
      const mockOrg = {
        id: 'uuid',
        name: 'John Doe Enterprises', // Should be the registered_name
        category: 'INDIVIDUAL',
      ...
      expect(result.name).toBe('John Doe Enterprises');
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      expect((result as any).registration.registered_name).toBe('John Doe Enterprises');
    });

    it('should create organization with name from registered_name when middle_name is null', async () => {
      const data = {
        category: 'INDIVIDUAL' as const,
        tax_classification: 'VAT' as const,
        tin: '001234567890',
        ...
        middle_name: null,
        last_name: 'Doe',
        registered_name: 'John Doe Freelance', // Mandatory
        trade_name: null,
        line_of_business: '6201',
        address_line: '123 Main St',
        region: 'NCR',
      ...
      const mockOrg = {
        id: 'uuid',
        name: 'John Doe Freelance', // Should be the registered_name
        category: 'INDIVIDUAL',
        tax_classification: 'VAT',
        tin: '001234567890',
        subcategory: 'SELF_EMPLOYED',
      ...
      const result = await repository.create(data);
      expect(result).toEqual(mockOrg);
      expect(result.name).toBe('John Doe Freelance');
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
  ...
  describe('NON_INDIVIDUAL category organization creation', () => {
    it('should create organization with name from registered_name', async () => {
      const data = {
        ...
        registered_name: 'ABC Corporation Inc.', // Mandatory for all categories
  ...
  describe('Validation errors', () => {
    ...
    it('should fail when INDIVIDUAL category is missing registered_name', async () => {
      const data = {
        category: 'INDIVIDUAL' as const,
        tax_classification: 'VAT' as const,
        tin: '001234567890',
